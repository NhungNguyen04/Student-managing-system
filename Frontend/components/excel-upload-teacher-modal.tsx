...
export function ExcelUploadTeacherModal({ isOpen, onClose, onUploadSuccess, setCheckReLoading }: ExcelUploadModalProps) {
  ...
  const processExcelData = async (data: any[]) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      let birthDate
      if (typeof teacher.birthDate === 'number') {
        // Excel stores dates as numbers, so we need to convert it
        birthDate = new Date((teacher.birthDate - 25569) * 86400 * 1000)
      } else {
        birthDate = new Date(teacher.birthDate)
      }

      if (isNaN(birthDate.getTime())) {
        console.error(`Invalid birth date for teacher: ${teacher.teachername}`)
        continue
      }
      ...
      formData.append("gender", teacher.gender === "Male" ? "1" : "2")
      formData.append("email", teacher.email)
      formData.append("subjectId", teacher.subjectId)
      
      try {
        const res = await teacherApi.createTeacher(formData)
        if (res.EC === 0) {
          successCount++
        } else {
          console.error(`Failed to create teacher: ${teacher.name}`, res.EM)
        }
      } catch (error) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const parsedData = jsonData.map((teacher: any) => ({
        ...teacher,
        birthDate: parseExcelDate(teacher.birthDate)
      }))
